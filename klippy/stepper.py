...
class PrinterStepper:
    def __init__(self, config):
        ...
        step_dist = config.getfloat("step_distance", above=0.0)
    ...
    def _dist_to_time(self, dist, start_velocity, accel):
        ...
        return math.sqrt(2.0 * dist / accel + time_offset ** 2) - time_offset

    def set_max_jerk(self, max_halt_velocity, max_accel):
        # Calculate the firmware's maximum halt interval time
        step_dist = self.get_step_dist()
        last_step_time = self._dist_to_time(step_dist, max_halt_velocity, max_accel)
        second_last_step_time = self._dist_to_time(
            2.0 * step_dist, max_halt_velocity, max_accel
        )
...
class PrinterRail:
    def __init__(
        self, config, need_position_minmax=True, default_position_endstop=None
    ):
        # Primary stepper
