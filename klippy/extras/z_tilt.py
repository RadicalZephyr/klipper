...
class RetryHelper:
    ...
    def start(self, params):
        self.max_retries = self.gcode.get_int(
            "RETRIES", params, default=self.default_max_retries, minval=0, maxval=30
        )
...
class ZTilt:
    def __init__(self, config):
        ...
        try:
            z_positions = [line.split(",", 1) for line in z_positions if line.strip()]
            self.z_positions = [
                (float(zp[0].strip()), float(zp[1].strip())) for zp in z_positions
            ]
        except:
            raise config.error(
                "Unable to parse z_positions in %s" % (config.get_name())
            )
