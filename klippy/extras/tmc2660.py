...
Fields["DRVCTRL"] = {"MRES": 0x0F, "DEDGE": 0x01 << 8, "intpol": 0x01 << 9}
...
FieldFormatters = {
    ...
    "DEDGE": (lambda v: "1(Both Edges Active)" if v else "0(Only Rising Edge active)"),
    ...
    "DISS2G": (
        lambda v: "%d(Short to GND protection %s)" % (v, "disabled" if v else "enabled")
    ),
    "MSTEP": (
        lambda v: "%d(%d, OA1 %s OA2)" % (v, v & 0xFF, "<=" if v & 0x100 else "=>")
    ),
...
class TMC2660CurrentHelper:
    ...
    def _calc_current_bits(self, current, vsense):
        ...
        cs = int(32 * current * self.sense_resistor * math.sqrt(2.0) / vref - 1.0 + 0.5)
