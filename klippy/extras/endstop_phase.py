...
class EndstopPhase:
    def __init__(self, config):
        ...
        self.endstop_phase = config.getint(
            "endstop_phase", None, minval=0, maxval=self.phases - 1
        )
        ...
        endstop_accuracy = config.getfloat("endstop_accuracy", None, above=0.0)
        if endstop_accuracy is None:
            self.endstop_accuracy = self.phases // 2 - 1
        elif self.endstop_phase is not None:
            self.endstop_accuracy = int(math.ceil(endstop_accuracy * 0.5 / step_dist))
        else:
    ...
    def align_endstop(self, pos):
        ...
        return int(pos / full_step + 0.5) * full_step + phase_offset
...
class EndstopPhases:
    ...
    def generate_stats(self, stepper_name, info):
        ...
        self.gcode.respond_info(
            "%s: endstop_phase=%d (range %d to %d)" % (stepper_name, best_phase, lo, hi)
        )
        return best_phase
