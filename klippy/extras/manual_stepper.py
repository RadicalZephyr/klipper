...
class ManualStepper:
    def __init__(self, config):
        ...
        self.accel = config.getfloat("accel", 0.0, minval=0.0)
        ...
        self.printer.register_event_handler("toolhead:motor_off", self.handle_motor_off)
    ...
    def do_set_position(self, setpos):
        self.stepper.set_position([setpos, 0.0, 0.0])

    def do_move(self, movepos, speed, accel):
        self.sync_print_time()
        cp = self.stepper.get_commanded_position()
        dist = movepos - cp
        accel_t, cruise_t, cruise_v = force_move.calc_move_time(dist, speed, accel)
        self.move_fill(
            self.cmove,
            self.next_cmd_time,
            accel_t,
    ...
    def cmd_MANUAL_STEPPER(self, params):
        ...
        speed = self.gcode.get_float("SPEED", params, self.velocity, above=0.0)
        accel = self.gcode.get_float("ACCEL", params, self.accel, minval=0.0)
        if homing_move:
