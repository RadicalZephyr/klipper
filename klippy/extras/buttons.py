...
class MCU_ADC_buttons:
    ...
    def handle_button(self, eventtime, value):
        ...
        if (
            (eventtime - self.last_debouncetime) >= ADC_DEBOUNCE_TIME
        ...
        if self.debug is True:
            logging.info("analog pin: %s value: %d" % (self.pin, int(value)))
...
class PrinterButtons:
    ...
    def register_adc_button(self, pin, min_val, max_val, pullup, callback, debug=False):
