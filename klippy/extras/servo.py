...
class PrinterServo:
    def __init__(self, config):
        ...
        initial_angle = config.getfloat("initial_angle", None, minval=0.0, maxval=360.0)
        if initial_angle is not None:
            self.initial_pwm_value = self._get_pwm_from_angle(initial_angle)
        else:
            initial_pulse_width = config.getfloat(
                "initial_pulse_width",
                None,
    ...
    def cmd_SET_SERVO(self, params):
        ...
        if "WIDTH" in params:
            self._set_pwm(
                print_time,
                self._get_pwm_from_pulse_width(self.gcode.get_float("WIDTH", params)),
            )
        else:
            if "ANGLE" in params:
                self._set_pwm(
                    print_time,
                    self._get_pwm_from_angle(self.gcode.get_float("ANGLE", params)),
                )
            else:
