...
class PrinterLCD:
    ...
    def get_status(self, eventtime):
        return {"progress": self.progress or 0, "message": self.message or ""}
    ...
    def screen_update_128x64(self, eventtime):
        ...
        if self.heater_bed is not None:
            info = self.heater_bed.get_status(eventtime)
            if info["target"]:
                self.animate_glyphs(eventtime, 0, extruder_count, "bed_heat", True)
            else:
        ...
        if self.fan is not None:
            info = self.fan.get_status(eventtime)
            self.animate_glyphs(eventtime, 10, 0, "fan", info["speed"] != 0.0)
    ...
    def draw_heater(self, x, y, info):
        ...
        if target and abs(temperature - target) > 2.0:
            self.draw_text(
                x, y, "%3.0f~right_arrow~%.0f~degrees~" % (temperature, target)
            )
        else:
    ...
    def draw_time(self, x, y, seconds):
        seconds = int(seconds)
        self.lcd_chip.write_text(
            x, y, "%02d:%02d" % (seconds // (60 * 60), (seconds // 60) % 60)
        )
    ...
    def cmd_M73(self, params):
        self.progress = min(100.0, max(0.0, self.gcode.get_float("P", params, 0.0)))
