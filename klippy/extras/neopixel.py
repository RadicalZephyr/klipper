...
class PrinterNeoPixel:
    def __init__(self, config):
        ...
        self.mcu.add_config_cmd(
            "config_neopixel oid=%d pin=%s" % (self.oid, pin_params["pin"])
        )
    ...
    def send_data(self, minclock=0):
        self.neopixel_send_cmd.send(
            [self.oid, self.color_data],
            minclock=minclock,
    ...
    def cmd_SET_LED(self, params):
        # Parse parameters
        red = self.gcode.get_float("RED", params, 0.0, minval=0.0, maxval=1.0)
        green = self.gcode.get_float("GREEN", params, 0.0, minval=0.0, maxval=1.0)
        blue = self.gcode.get_float("BLUE", params, 0.0, minval=0.0, maxval=1.0)
        index = self.gcode.get_int(
            "INDEX", params, None, minval=1, maxval=self.chain_count
        )
