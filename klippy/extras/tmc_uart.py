...
class MCU_analog_mux:
    def __init__(self, mcu, cmd_queue, select_pins_desc):
        ...
        select_pin_params = [
            ppins.lookup_pin(spd, can_invert=True)
        ...
        for oid, pin, value in zip(self.oids, self.pins, self.pin_values):
            self.mcu.add_config_cmd(
                ...
                " value=%d default_value=0 max_duration=0" % (oid, pin, value)
            )
    ...
    def get_instance_id(self, select_pins_desc):
        ...
        select_pin_params = [
            ppins.parse_pin(spd, can_invert=True) for spd in select_pins_desc.split(",")
        ]
...
class MCU_TMC_uart_bitbang:
    ...
    def build_config(self):
        bit_ticks = int(self.mcu.get_adjusted_freq() / 9000.0)
        self.mcu.add_config_cmd(
    ...
    def reg_write(self, instance_id, addr, reg, val, print_time=None):
        ...
        self.tmcuart_send_cmd.send_with_response(
            [self.oid, msg, 0], "tmcuart_response", self.oid, minclock=minclock
        )
