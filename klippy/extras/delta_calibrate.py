...
DeltaParams = collections.namedtuple(
    "DeltaParams",
    ["radius", "angles", "arms", "endstops", "stepdists", "towers", "abs_endstops"],
)
...
def build_delta_params(params):
    ...
    towers = [(math.cos(a) * radius, math.sin(a) * radius) for a in radian_angles]
    # Calculate the absolute Z height of each tower endstop
    radius2 = radius ** 2
    abs_endstops = [e + math.sqrt(a ** 2 - radius2) for e, a in zip(endstops, arms)]
    return DeltaParams(radius, angles, arms, endstops, stepdists, towers, abs_endstops)
...
def calc_stable_position(coord, delta_params):
    ...
    return [
        (ep - sp) / sd for sd, ep, sp in zip(dp.stepdists, dp.abs_endstops, steppos)
    ]
...
class DeltaCalibrate:
    ...
    def save_state(self, probe_positions, distances, params):
        ...
        for axis in "abc":
            configfile.set(
                "stepper_" + axis, "angle", "%.6f" % (params["angle_" + axis],)
            )
            configfile.set(
                "stepper_" + axis, "arm_length", "%.6f" % (params["arm_" + axis],)
            )
            configfile.set(
                "stepper_" + axis,
                "position_endstop",
    ...
    def probe_finalize(self, offsets, positions):
        ...
        probe_positions = [
            (z_offset, calc_stable_position(p, delta_params)) for p in positions
        ]
        # Perform analysis
    ...
    def calculate_params(self, probe_positions, distances):
        ...
        self.gcode.respond_info(
            ...
            % (
                new_params["endstop_a"],
                new_params["angle_a"],
                new_params["arm_a"],
                new_params["endstop_b"],
                new_params["angle_b"],
                new_params["arm_b"],
    ...
    def cmd_DELTA_ANALYZE(self, params):
        ...
        for name, count in args.items():
            ...
            try:
                ...
                raise self.gcode.error("Unable to parse parameter '%s'" % (name,))
            if len(parts) != count:
                raise self.gcode.error(
