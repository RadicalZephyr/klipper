...
class TMC5160CurrentHelper:
    ...
    def _calc_current_bits(self, current):
        cs = int(
            32.0 * current * self.sense_resistor * math.sqrt(2.0) / VREF - 1.0 + 0.5
        )
    ...
    def _calc_current_from_field(self, field_name):
        ...
        current = (bits + 1) * VREF / (32.0 * math.sqrt(2.0) * self.sense_resistor)
    ...
    def cmd_SET_TMC_CURRENT(self, params):
        ...
        if "HOLDCURRENT" in params:
            hold_current = gcode.get_float(
                "HOLDCURRENT", params, above=0.0, maxval=MAX_CURRENT
            )
        else:
        ...
        if "HOLDCURRENT" not in params and "CURRENT" not in params:
            # Query only
            gcode.respond_info(
                "Run Current: %0.2fA Hold Current: %0.2fA" % (run_current, hold_current)
            )
            return
