...
class BaseSensor(object):
    ...
    def cmd_QUERY_FILAMENT_SENSOR(self, params):
        raise NotImplementedError("Sensor must implement cmd_QUERY_FILAMENT_SENSOR")

    cmd_SET_FILAMENT_SENSOR_help = "Sets the filament sensor on/off"

    def cmd_SET_FILAMENT_SENSOR(self, params):
        raise NotImplementedError("Sensor must implement cmd_SET_FILAMENT_SENSOR")
...
class SwitchSensor(BaseSensor):
    ...
    def _handle_ready(self):
        self.start_time = self.reactor.monotonic() + 2.0
    ...
    def _button_handler(self, eventtime, state):
        ...
        if state:
            ...
            if (
                ...
                and (eventtime - self.last_cb_event_time) > self.event_delay
            ):
                self.last_cb_event_time = eventtime
                logging.info(
                    "switch_sensor: insert event detected, Time %.2f", eventtime
                )
            ...
            self.last_cb_event_time = eventtime
            logging.info("switch_sensor: runout event detected, Time %.2f", eventtime)
