...
class PrinterProbe:
    ...
    def _probe(self, speed):
        ...
        try:
            homing_state.homing_move(
                pos, endstops, speed, probe_pos=True, verify_movement=verify
            )
    ...
    def run_probe(self, params={}):
        speed = self.gcode.get_float("PROBE_SPEED", params, self.speed, above=0.0)
        sample_count = self.gcode.get_int(
            "SAMPLES", params, self.sample_count, minval=1
        )
        sample_retract_dist = self.gcode.get_float(
            "SAMPLE_RETRACT_DIST", params, self.sample_retract_dist, above=0.0
        )
        samples_tolerance = self.gcode.get_float(
            "SAMPLES_TOLERANCE", params, self.samples_tolerance, minval=0.0
        )
        ...
        while len(positions) < sample_count:
            ...
            if max(z_positions) - min(z_positions) > samples_tolerance:
                if retries >= samples_retries:
                    raise homing.CommandError("Probe samples exceed samples_tolerance")
                self.gcode.respond_info("Probe samples exceed tolerance. Retrying...")
                retries += 1
                positions = []
            # Retract
    ...
    def cmd_QUERY_PROBE(self, params):
        ...
        self.gcode.respond_info("probe: %s" % (["open", "TRIGGERED"][not not res],))
    ...
    def cmd_PROBE_ACCURACY(self, params):
        ...
        self.gcode.respond_info(
            ...
            % (pos[0], pos[1], pos[2], sample_count, sample_retract_dist, speed)
        )
    ...
    def cmd_PROBE_CALIBRATE(self, params):
        ...
        manual_probe.ManualProbeHelper(
            self.printer, params, self.probe_calibrate_finalize
        )
...
class ProbePointsHelper:
    def __init__(self, config, finalize_callback, default_points=None):
        ...
        if default_points is None or config.get("points", None) is not None:
            ...
            try:
                ...
                self.probe_points = [
                    (float(p[0].strip()), float(p[1].strip())) for p in points
                ]
            except:
                raise config.error("Unable to parse probe points in %s" % (self.name))
    ...
    def _manual_probe_start(self):
        ...
        if not done:
            manual_probe.ManualProbeHelper(
                self.printer, {}, self._manual_probe_finalize
            )
