...
class GCodeMacro:
    def __init__(self, config):
        ...
        self.kwparams = {
            o[len(prefix) :].upper(): config.get(o)
    ...
    def cmd_SET_GCODE_VARIABLE(self, params):
        ...
        if variable not in self.variables:
            if variable in self.kwparams:
                ...
                return
            raise self.gcode.error("Unknown gcode_macro variable '%s'" % (variable,))
        try:
            literal = ast.literal_eval(value)
        except ValueError as e:
            raise self.gcode.error("Unable to parse '%s' as a literal" % (value,))
