...
class ArcSupport:
    ...
    def cmd_G2(self, params):
        ...
        asR = float(params.get("R", 0.0))  # radius
        asI = float(params.get("I", 0.0))
        asJ = float(params.get("J", 0.0))

        asE = float(params.get("E", 0.0))
        ...
        if asX is None or asY is None:
            ...
            if asI != 0 or asJ != 0:
                coords = self.planArc(
                    currentPos, [asX, asY, 0.0, 0.0], [asI, asJ], clockwise
                )
            ###############################
