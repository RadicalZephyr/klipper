...
class PrinterExtruder:
    def __init__(self, config, extruder_num):
        ...
        self.pellet_control = pellet_control.PelletControl(config)
    ...
    def check_move(self, move):
        ...
        if not move.is_kinematic_move or move.extrude_r < 0.0:
            ...
            inv_extrude_r = 1.0 / abs(move.extrude_r)
            move.limit_speed(
    ...
    def calc_junction(self, prev_move, move):
        ...
        if extrude or prev_extrude:
            if not extrude or not prev_extrude:
                ...
                return 0.0
            if (
                move.extrude_r > prev_move.extrude_r * EXTRUDE_DIFF_IGNORE
                or prev_move.extrude_r > move.extrude_r * EXTRUDE_DIFF_IGNORE
            ) and abs(move.move_d * prev_move.extrude_r - extrude) >= 0.001:
        ...
        for i in range(flush_count):
            ...
            for j in range(i + 1, flush_count):
                ...
                if fmove.cruise_v > max_corner_v:
                    if not max_corner_v and not fmove.accel_t and not fmove.cruise_t:
    ...
    def move(self, print_time, move):
        self.pellet_control.update_next_movement_time(print_time)
        ...
        extra_accel_v = extra_decel_v = 0.0
    ...
    def cmd_SET_PRESSURE_ADVANCE(self, params):
        ...
        pressure_advance = gcode.get_float(
            "ADVANCE", params, self.pressure_advance, minval=0.0
        )
        pressure_advance_lookahead_time = gcode.get_float(
            "ADVANCE_LOOKAHEAD_TIME",
            params,
