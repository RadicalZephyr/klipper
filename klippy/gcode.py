...
class GCodeParser:
    ...
    def get_str(
        ...
        if name not in params:
            if default is self.sentinel:
                raise self.error(
                    "Error on '%s': missing %s" % (params["#original"], name)
                )
            return default
        try:
    ...
    def get_int(self, name, params, default=sentinel, minval=None, maxval=None):
        return self.get_str(
            name, params, default, parser=int, minval=minval, maxval=maxval
        )
    ...
    def _set_temp(self, params, is_bed=False, wait=False):
        temp = self.get_float("S", params, 0.0)
        heater = None
        if is_bed:
    ...
    def cmd_G1(self, params):
        ...
        try:
            ...
            if "F" in params:
                ...
                if gcode_speed <= 0.0:
                    raise self.error("Invalid speed in '%s'" % (params["#original"],))
                self.speed = gcode_speed * self.speed_factor
        except ValueError as e:
            raise self.error("Unable to parse move '%s'" % (params["#original"],))
        self.move_with_transform(self.last_position, self.speed)

    def cmd_G4(self, params):
        # Dwell
        if "S" in params:
            delay = self.get_float("S", params, minval=0.0)
        else:
            delay = self.get_float("P", params, 0.0, minval=0.0) / 1000.0
    ...
    def cmd_M220(self, params):
        ...
        value = self.get_float("S", params, 100.0, above=0.0) / (60.0 * 100.0)
        self.speed = self._get_gcode_speed() * value
        self.speed_factor = value

    def cmd_M221(self, params):
        # Set extrude factor override percentage
        new_extrude_factor = self.get_float("S", params, 100.0, above=0.0) / 100.0
    ...
    def cmd_SET_GCODE_OFFSET(self, params):
        ...
        if self.get_int("MOVE", params, 0):
            speed = self.get_float("MOVE_SPEED", params, self.speed, above=0.0)
    ...
    def cmd_M206(self, params):
        # Offset axes
        offsets = {
            self.axis2pos[a]: -self.get_float(a, params) for a in "XYZ" if a in params
        }
    ...
    def cmd_M106(self, params):
        # Set fan speed
        self._set_fan_speed(self.get_float("S", params, 255.0, minval=0.0) / 255.0)
    ...
    def cmd_GET_POSITION(self, params):
        ...
        stepper_pos = " ".join(
            ["%s:%.6f" % (s.get_name(), s.get_commanded_position()) for s in steppers]
        )
        kinematic_pos = " ".join(
            ["%s:%.6f" % (a, v) for a, v in zip("XYZE", kin.calc_position())]
        )
        toolhead_pos = " ".join(
            ["%s:%.6f" % (a, v) for a, v in zip("XYZE", self.toolhead.get_position())]
        )
        gcode_pos = " ".join(
