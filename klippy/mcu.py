...
class MCU_stepper:
    def __init__(self, mcu, pin_params):
        ...
        self._stepqueue = ffi_main.gc(
            self._ffi_lib.stepcompress_alloc(oid), self._ffi_lib.stepcompress_free
        )
    ...
    def _build_config(self):
        ...
        min_stop_interval = max(0.0, self._min_stop_interval - max_error)
        ...
        self._get_position_cmd = self._mcu.lookup_command("stepper_get_position oid=%c")
    ...
    def set_ignore_move(self, ignore_move):
        was_ignore = self._itersolve_gen_steps is not self._ffi_lib.itersolve_gen_steps
        if ignore_move:
    ...
    def note_homing_end(self, did_trigger=False):
        ...
        ret = self._ffi_lib.stepcompress_queue_msg(self._stepqueue, data, len(data))
        if ret:
            raise error("Internal error in stepcompress")
...
class MCU_endstop:
    ...
    def __init__(self, mcu, pin_params):
        ...
        self._mcu.register_config_callback(self._build_config)
        self._min_query_time = self._last_sent_time = 0.0
        self._next_query_print_time = self._end_home_time = 0.0
    ...
    def _build_config(self):
        ...
        self._mcu.add_config_cmd(
            ...
            " rest_ticks=0 pin_value=0" % (self._oid,),
            is_init=True,
        )
    ...
    def home_start(
        ...
        self._mcu.register_response(
            self._handle_endstop_state, "endstop_state", self._oid
        )
        self._home_cmd.send(
            [
                self._oid,
...
class MCU_pwm:
    ...
    def _build_config(self):
        ...
        if self._hardware_pwm:
            ...
            if self._start_value not in [0.0, 1.0]:
...
class CommandWrapper:
    ...
    def send_with_response(self, data=(), response=None, response_oid=None, minclock=0):
        minsystime = 0.0
        if minclock:
        ...
        try:
            return src.get_response(
                [cmd], self._cmd_queue, minclock=minclock, minsystime=minsystime
            )
...
class MCU:
    ...
    def __init__(self, config, clocksync):
        ...
        if baud:
            ...
            self._restart_method = config.getchoice("restart_method", rmethods, None)
    ...
    def _check_config(self):
        ...
        self._steppersync = self._ffi_lib.steppersync_alloc(
            self._serial.serialqueue,
            self._stepqueues,
            len(self._stepqueues),
            self._move_count,
        )
        self._ffi_lib.steppersync_set_time(self._steppersync, 0.0, self._mcu_freq)

    def _connect(self):
        if self.is_fileoutput():
            self._connect_file()
        else:
            if self._restart_method == "rpi_usb" and not os.path.exists(
                self._serialport
            ):
        ...
        if (
            self._restart_method is None
            and (self._reset_cmd is not None or self._config_reset_cmd is not None)
    ...
    def stats(self, eventtime):
        ...
        return (
            False,
            " ".join(
                [msg, self._serial.stats(eventtime), self._clocksync.stats(eventtime)]
            ),
        )
