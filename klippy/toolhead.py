...
class Move:
    ...
    def calc_junction(self, prev_move):
        ...
        R = self.toolhead.junction_deviation * sin_theta_d2 / (1.0 - sin_theta_d2)
...
class ToolHead:
    def __init__(self, config):
        ...
        # Print time tracking
        self.buffer_time_low = config.getfloat("buffer_time_low", 1.000, above=0.0)
        self.buffer_time_high = config.getfloat(
            "buffer_time_high", 2.000, above=self.buffer_time_low
        )
        self.buffer_time_start = config.getfloat("buffer_time_start", 0.250, above=0.0)
        self.move_flush_time = config.getfloat("move_flush_time", 0.050, above=0.0)
        self.print_time = 0.0
        self.special_queuing_state = "Flushed"
        self.need_check_stall = -1.0
        self.flush_timer = self.reactor.register_timer(self._flush_handler)
        self.move_queue.set_flush_time(self.buffer_time_high)
        self.last_print_start_time = 0.0
    ...
    def get_next_move_time(self):
        ...
        if self.special_queuing_state == "Drip":
            ...
            while 1:
                ...
                if wait_time <= 0.0 or self.mcu.is_fileoutput():
                    return self.print_time
    ...
    def _flush_handler(self, eventtime):
        try:
            ...
            self.printer.invoke_shutdown('Exception in flush_handler %s' % (e))
    ...
    def drip_move(self, newpos, speed):
        ...
        inv_num_moves = 1.0 / float(num_moves)
    ...
    def stats(self, eventtime):
        ...
        return (
            ...
            % (self.print_time, max(buffer_time, 0.0), self.print_stall),
        )
    ...
    def get_status(self, eventtime):
        ...
        if buffer_time > -1.0 or not self.special_queuing_state:
            status = "Printing"
        else:
            status = "Ready"
        return {
    ...
    def get_max_axis_halt(self):
        ...
        return min(
            self.max_velocity, math.sqrt(8.0 * self.junction_deviation * self.max_accel)
        )

    def _calc_junction_deviation(self):
        scv2 = self.square_corner_velocity ** 2
        self.junction_deviation = scv2 * (math.sqrt(2.0) - 1.0) / self.max_accel
        self.max_accel_to_decel = min(self.requested_accel_to_decel, self.max_accel)
    ...
    def cmd_SET_VELOCITY_LIMIT(self, params):
        ...
        gcode = self.printer.lookup_object("gcode")
        max_velocity = gcode.get_float("VELOCITY", params, self.max_velocity, above=0.0)
        max_accel = gcode.get_float("ACCEL", params, self.max_accel, above=0.0)
        square_corner_velocity = gcode.get_float(
            "SQUARE_CORNER_VELOCITY", params, self.square_corner_velocity, minval=0.0
        )
        self.requested_accel_to_decel = gcode.get_float(
            "ACCEL_TO_DECEL", params, self.requested_accel_to_decel, above=0.0
        )
        ...
        self.square_corner_velocity = min(
            square_corner_velocity, self.config_square_corner_velocity
        )
        self._calc_junction_deviation()
        msg = (
            "max_velocity: %.6f\n"
    ...
    def cmd_M204(self, params):
        ...
        if "S" in params:
            # Use S for accel
            accel = gcode.get_float("S", params, above=0.0)
        elif "P" in params and "T" in params:
            # Use minimum of P and T for accel
            accel = min(
                gcode.get_float("P", params, above=0.0),
                gcode.get_float("T", params, above=0.0),
            )
        else:
